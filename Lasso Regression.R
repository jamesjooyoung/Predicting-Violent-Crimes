## Conducting linear regression / L1 Norm (Lasso)
...
for (i in 1:5) {
  ...
  # fitting the model
  fit <- cv.glmnet(train_x, train_y, alpha = 1, nfolds = 5, type.measure = "mse", 
                   lambda = seq(0.001, 0.1, by = 0.001), parallel = TRUE)
  # finding best lambda
  lambda_opt <- fit$lambda.min
  # creating best lasso regression object w/ train_data
  lasso <- glmnet(train_x, train_y, alpha = 1, lambda = lambda_opt)
  # finding predictions on test data (for Rsquared calculation)
  y_pred <- predict(lasso, newx = test_x)
...
avg_results_lasso <- data.frame(avg_mae = numeric(),
                                avg_rmse = numeric())

#store results
avg_results_lasso[1,1] <- mean(unmelted_df$MAE)
avg_results_lasso[1,2] <- mean(unmelted_df$RMSE)
...
# creating lasso coefficient dataframe
lasso.mod =glmnet(x,y, alpha =1, lambda = seq(0.001, 0.1, by = 0.001))#this will give 80 values of lambda
beta=coef(lasso.mod)
...
for (i in 1:length(lasso.mod$lambda)) {
  ...
  coef_i <- as.data.frame(t(lasso.mod$beta[, i]))
  
  # add lambda value to coefficients dataframe
  coef_i$lambda <- lasso.mod$lambda[i]
